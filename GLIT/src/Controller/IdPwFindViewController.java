...
public class IdPwFindViewController {
	...
	public void findIdBtn(ActionEvent event) {
		...
		try {
			Response resp = Jsoup.connect(URL).method(Method.POST).ignoreContentType(true)
					.ignoreHttpErrors(true).header("API_Version", "1.0")
					.data("email", emailInputToFindID.getText()).execute();
	...
	public void findPwBtn(ActionEvent event) {
		...
		try {
			Response resp = Jsoup.connect(URL).method(Method.POST).ignoreContentType(true)
					...
					.data("email", emailInputToFindPW.getText()).execute();
